...
import pageobjects.PositionScreen;
import utils.Commons;

public class TestPositionScreenLoadTime {
...
// 		precondition trade 5 scripts
	...
	public void Position_screen_time_load() throws InterruptedException {
		...
		PositionScreen Position = new PositionScreen(Driver);
		...
		for (int i = 1; i <= 10; i++) {
			...
			WebElement tabposition = wait.until(ExpectedConditions.elementToBeClickable(Position.positions));
	...
	public void Position_screen_orderform_time_load() throws InterruptedException {
		...
		tapWithActions(Driver, 753, 2160);
		for (int i = 1; i <= 10; i++) {
			Position.khoobsuratscript.click();
			Position.exitbutton.click();
